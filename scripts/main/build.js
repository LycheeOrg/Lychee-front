...
build.album = function (data, disabled = false) {
	...
	if (album.isUploadable() && !disabled) {
		html += lychee.html`
				<div class='badges'>
					<a class='badge ${
						data.nsfw === "1" ? "badge--nsfw" : ""
					} icn-warning'>${build.iconic("warning")}</a>
					<a class='badge ${
						data.star === "1" ? "badge--star" : ""
					} icn-star'>${build.iconic("star")}</a>
					<a class='badge ${data.public === "1" ? "badge--visible" : ""} ${
			data.visible === "1" ? "badge--not--hidden" : "badge--hidden"
		} icn-share'>${build.iconic("eye")}</a>
					<a class='badge ${
						data.unsorted === "1" ? "badge--visible" : ""
					}'>${build.iconic("list")}</a>
					<a class='badge ${
						data.recent === "1" ? "badge--visible badge--list" : ""
					}'>${build.iconic("clock")}</a>
					<a class='badge ${
						data.password === "1" ? "badge--visible" : ""
					}'>${build.iconic("lock-locked")}</a>
					<a class='badge ${data.tag_album === "1" ? "badge--tag" : ""}'>${build.iconic(
			"tag"
		)}</a>
				</div>
				`;
	...
	if (
		...
		(data.hasOwnProperty("has_albums") && data.has_albums === "1")
	) {
		html += lychee.html`
...
build.photo = function (data, disabled = false) {
	...
	if (data.thumbUrl === "uploads/thumb/" && isVideo) {
		...
		if (data.small !== "") {
			if (data.hasOwnProperty("small2x") && data.small2x !== "") {
				thumb2x = `data-srcset='${data.small} ${parseInt(
					data.small_dim,
					10
				)}w, ${data.small2x} ${parseInt(data.small2x_dim, 10)}w'`;
			}
			...
			if (data.hasOwnProperty("medium2x") && data.medium2x !== "") {
				thumb2x = `data-srcset='${data.medium} ${parseInt(
					data.medium_dim,
					10
				)}w, ${data.medium2x} ${parseInt(data.medium2x_dim, 10)}w'`;
			}
	...
	html += lychee.html`
			<div class='photo ${disabled ? `disabled` : ``}' data-album-id='${
		data.album
	}' data-id='${data.id}' data-tabindex='${tabindex.get_next_tab_index()}'>
				${thumbnail}
				<div class='overlay'>
					<h1 title='$${data.title}'>$${data.title}</h1>
			`;
	...
	if (data.takedate !== "")
		html += lychee.html`<a><span title='Camera Date'>${build.iconic(
			"camera-slr"
		)}</span>${data.takedate}</a>`;
	...
	if (album.isUploadable()) {
		html += lychee.html`
				<div class='badges'>
					...
					<a class='badge ${
						data.public === "1" && album.json.public !== "1"
...
build.overlay_image = function (data) {
	...
	// fall back to exif data if there is no description
		...
		html += lychee.html`
			...
			<p>${data.shutter.replace("s", "sec")} at ${data.aperture.replace(
			"f/",
			"&fnof; / "
		)}, ${lychee.locale["PHOTO_ISO"]} ${data.iso}<br>
...
build.imageview = function (data, visibleControls, autoplay) {
	...
	if (data.type.indexOf("video") > -1) {
		html += lychee.html`<video width="auto" height="auto" id='image' controls class='${
		...
		if (data.livePhotoUrl === "" || data.livePhotoUrl === null) {
			...
			$(".photo").each(function () {
				if (
					...
					if (thumbimg.length > 0) {
						thumb = thumbimg[0].currentSrc
							? thumbimg[0].currentSrc
							: thumbimg[0].src;
						return false;
					}
				}
			...
			if (data.medium !== "") {
				...
				if (data.hasOwnProperty("medium2x") && data.medium2x !== "") {
					medium = `srcset='${data.medium} ${parseInt(
						data.medium_dim,
						10
					)}w, ${data.medium2x} ${parseInt(data.medium2x_dim, 10)}w'`;
				}
			...
			if (data.medium !== "") {
				...
				img = `<div id='livephoto' data-live-photo data-proactively-loads-video='true' data-photo-src='${
					data.url
				}' data-video-src='${data.livePhotoUrl}'  style='width: ${
					data.width
				}px; height: ${
					data.height
	...
	html += `
			<div class='arrow_wrapper arrow_wrapper--previous'><a id='previous'>${build.iconic(
				"caret-left"
			)}</a></div>
			<div class='arrow_wrapper arrow_wrapper--next'><a id='next'>${build.iconic(
				"caret-right"
			)}</a></div>
			`;
...
build.uploadModal = function (title, files) {
	...
	while (i < files.length) {
		...
		if (file.name.length > 40)
			file.name =
				file.name.substr(0, 17) +
				"..." +
				file.name.substr(file.name.length - 20, 20);
...
build.tags = function (tags) {
	...
	let searchable =
		lychee.publicMode === false || lychee.public_search === true;
	...
	if (tags !== "") {
		...
		tags.forEach(function (tag, index) {
			if (editable) {
				html += lychee.html`<a class='${a_class}'>$${tag}<span data-index='${index}'>${build.iconic(
					"x"
				)}</span></a>`;
			} else {
...
build.u2f = function (credential) {
	return lychee.html`<div class="u2f_view_line">
			...
			<a id="CredentialDelete${
				credential.id
			}"  class="basicModal__button basicModal__button_DEL">Delete</a>
		</div>
		`;
};
